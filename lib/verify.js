...
module.exports = async (pluginConfig, { logger }) => {
  ...
  try {
    ...
    logger.error(`Error running "sentry-cli info".`, err);
    errors.push(new Error(`Unable to use Sentry CLI: ${err.message}`));
  }

  if (errors.length > 0 && pluginConfig.allowSentryFailure) {
    throw new AggregateError(errors);
  }

  if (pluginConfig.allowSentryFailure) {
      logger.log(`Sentry verify failed, but this is allowed by config. Err: ${errors.map(e => e.message).join(';')}`);
      return;
    }
};
